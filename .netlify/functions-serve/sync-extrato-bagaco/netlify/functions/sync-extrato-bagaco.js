...
// netlify/functions/sync-extrato-bagaco.ts
var sync_extrato_bagaco_exports = {};
__export(sync_extrato_bagaco_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(sync_extrato_bagaco_exports);
...
// netlify/functions/sync-extrato-bagaco.ts
var supabaseUrl = process.env.SUPABASE_URL || "https://xpcxuonqffewtsmwlato.supabase.co";
...
var handler = async (event) => {
  ...
  try {
    ...
    const payload = extrato.map((item) => ({
      id: item.id,
      data: item.data,
      hora: item.hora,
      tipo: item.tipo,
    ...
    const { error } = await supabase.from("extrato_bagaco").upsert(payload, { onConflict: "id" });
    if (error) {
      throw error;
    }
    console.log(`\u2705 Extrato de baga\xE7o sincronizado: ${payload.length} registros.`);
    return {
      statusCode: 200,
      headers,
...
//# sourceMappingURL=sync-extrato-bagaco.js.map
